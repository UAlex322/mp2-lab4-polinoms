...
private:
	...
	struct Monomial {
		...
		Monomial operator*(const Monomial &mon) const {
			...
			// —тепень при некоторой переменной больше 9
		...
		// ƒл€ задани€ пор€дка при сортировке
		bool operator<=(const Monomial &mon) const {
			return power <= mon.power;
		}
	};

	void reduction(List<Monomial> &list); // приведение подобных слагаемых в многочлене с упор€доченными слагаемыми
	void parse(const std::string &s); // парсинг строки с многочленом
	List<Monomial> list;

public:

	// ¬спомогательна€ структура дл€ создани€ мономов через конкретные значени€, а не парсинг строки
	struct Mono {
		double value;
