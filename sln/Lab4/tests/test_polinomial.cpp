...
TEST(Polinomial, initializer_list_constructor_is_correct) {
	// Конструктор от initializer_list напрямую вставляет мономы и не изменяет входные данные.
	// Вставка в List работает корректно, поэтому математически конструктор работает корректно.
	// Проверить напрямую равенство переданных в конструктор значений и значений в многочлене по-честному невозможно, т.к.
	// прямой доступ к списку внутри Polinomial извне закрыт.
